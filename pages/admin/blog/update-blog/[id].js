...
import { getIdBlogCategoryUrl, getIdBlogUrl, getUpdateBlogCategoryUrl, getUpdateBlogUrl } from "@/utils/urls";
...
const UpdateCategory = () => {
  ...
  const getBlogById = async () => {
    const res = await (
      await fetch(`${getIdBlogUrl}/${router.query.id}`, {
        method: "GET",
        headers: {
    ...
    if (res.error) {
      ...
      setBlog({
        ...
        html_description: res.blog.html_description,
      });

      setContent(res.blog.html_description)
    }
  };
  ...
  const onSubmitHandle = async (e) => {
    ...
    if (title && html_description) {
      const res = await (
        await fetch(`${getUpdateBlogUrl}/${router.query.id}`, {
          method: "PUT",
